...
class PersonaControllerMockMvcIT extends AbstractIntegration{
	...
	void createPersonaPost_isObtainedWithGet() throws Exception {
        ...
        this.mockMvc.perform(delete("/persona")
        .contentType("application/json")
        .content(objectMapper.writeValueAsString(persona)));

         // comprobamos que no existen personas
		this.mockMvc.perform(get("/personas"))
        .andDo(print())
        .andExpect(status().is2xxSuccessful())
        .andExpect(content().contentType("application/json"))
        .andExpect(jsonPath("$", hasSize(0)));  // comprueba que el tamanyo sea 1
        
    }


    @Test
    @DisplayName("Create a persona, update it and get it correctly")
	void createPersonaPost_isObtainedWithGetAndUpdateit() throws Exception {
        Persona persona = new Persona();
        persona.setNombre("Alumno"); 
        persona.setDni("122");
        persona.setEdad(16);
        ...
        // Cambiamos el DNI para actualizar 
        persona.setDni("444");

        // actualizamos la persona
        this.mockMvc.perform(put("/persona")
        .contentType("application/json")
        .content(objectMapper.writeValueAsString(persona)))
        .andExpect(status().isOk())
        .andExpect(status().is2xxSuccessful());

         // comprobamos que existe la persona actualizada
		this.mockMvc.perform(get("/personas"))
        .andExpect(status().is2xxSuccessful())
        .andExpect(content().contentType("application/json"))
        .andExpect(jsonPath("$[0].dni").value("444"));  // comprueba que el tamanyo sea 1
