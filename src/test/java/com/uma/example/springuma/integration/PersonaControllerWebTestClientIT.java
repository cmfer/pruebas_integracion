...
class PersonaControllerWebTestClientIT {
    ...
    @Test
    @DisplayName("Create a persona and delete it correctly")
    public void createPersonaPost_DeleteitCorrectly() {
        ...
        // obtiene la persona con ID 1
        client.get().uri("/persona/1")
            .exchange()
            .expectStatus().isOk()
            .expectBody().jsonPath("$.dni").isEqualTo(persona.getDni());

        // elimina una persona
        client.method(HttpMethod.DELETE).uri("/persona")
        .body(Mono.just(persona), Persona.class)
        .exchange()
